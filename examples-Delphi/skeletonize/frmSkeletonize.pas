...
begin
  ...
  ksize := CvSize_(3,3);
  anchor := CvPoint_(1,1);
  element := pCvgetStructuringElement(MORPH_CROSS, ksize, anchor);
  done:=1;
  n:=0;
  ...
  for i:=1 to 500 do
  begin
      pCverode(imgInput, eroded, element, erodeAnchor, 1, BORDER_CONSTANT, borderValue);
      pCvdilate(eroded, temp, element, erodeAnchor, 1, BORDER_CONSTANT, borderValue);
      pCvsubtract(imgInput, temp, temp, pCvDefaultMat, -1);
      w:=pCvMatGetType(skel);
      w:=pCvMatGetType(temp);
      pCvbitwise_or(skel, temp, skel, pCvDefaultMat);
      pCvMatCopy(eroded, imgInput);
      done := pcvCountNonZero(imgInput);

      if done=0 then Break;
  end;
  pCvthreshold(skel, skel, 25, 255, THRESH_BINARY);
