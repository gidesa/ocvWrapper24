...
int main( int argc, char** argv )
{
        ...
        )
    {
 		...
 		CvKNearest_t*  knn = pCvCvKNearestCreate();
        // train kNN classifier (using training data)

		Mat_t* emptymat = pCvMatCreateEmpty();
		pCvCvKNearesttrain(knn, training_data, training_responses, emptymat, false, 32, false);
        ...
        for (int tsample = 0; tsample < nrows; tsample++)
        {
			...
			result = pCvCvKNearestfind_nearest(  knn,  test_sample, 7, emptymat, emptymat, emptymat);
